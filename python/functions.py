# -*- coding: utf-8 -*-
"""
Módulo de Visualização e Tratamento de Dados do projeto
...
class MegaSenaViz:
    """
    Classe de visualização e tratamento de dados do projeto Mega-Sena.
 
    Centraliza as cores, fontes e tamanhos padrão do projeto,
    expondo cada gráfico como um método de instância.
 
    Uso:
        viz = MegaSenaViz()
        viz.dispersao(df, 'col_x', 'col_y', 'Título')
        viz.vacancia(df)
    """
    ...
    def __init__(self):
        # Paleta de cores
        self.cor_principal = '#00441b'
        self.cor_secundaria = '#7F8C8D'
        self.palette_seq = "magma"
 
        # Tipografia
        self.font_titulo = 20
        self.font_subtitulo = 14
        self.font_rodape = 10
        self.font_eixo = 12
 
        # LAyout 
        self.figsize_padrao = (20, 10)
        self.rodape_autoria = (
            'Dados extraídos da Caixa Econômica Federal'
            ' e SIDRA (IBGE) | Elaborado por Lucas Reges Lima')
    ...
    def dispersao(self,dados,eixo_x,eixo_y):
    ...
    def comparativo_SQQ(self, win6,win5,win4):
        ...
        fig.text(0.5, 0.01, "*As escalas variam drasticamente entre Sena e Quadra", 
             ha="center", fontsize=self.font_eixo, style='italic', bbox={'facecolor':'orange', 'alpha':0.2, 'pad':5})
        plt.show()
    ...
    def vacancia(self, dataframe):
        ...
        for i in range(len(nulos)):
            plt.text(x = i + 0.5,y=-1.5 , s = nulos.iloc[i], ha='center') 
    ...
    # ==========================================
    def geo_x_bar(self, dataframe, col_valor, colors, titulo, subtitulo, legenda, label_gradiente):
        ...
        """ 
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize= self.figsize_padrao, gridspec_kw={'width_ratios': [2.2, 1]}) #quadro com dois plots novamente
        ...
        ax1.text(0.05, 1.05, titulo, 
                transform=ax1.transAxes, fontsize=self.font_titulo, fontweight='bold', color= mcolors.to_hex(cor_titulo))
        ax1.text(0.05, 0.97, subtitulo, 
                transform=ax1.transAxes, fontsize=self.font_subtitulo, color=self.cor_secundaria)
    ...
    # ==========================================
    def single_barplot(self, X, Y, titulo:str, subtitulo:str):
        """
        ...
        plt.figure(figsize=self.figsize_padrao)
        ax = sns.barplot(x=X, y=Y, color= self.cor_principal)

        plt.xticks(rotation=45)

        ax.text(0.0, 1.12, titulo, 
                 transform=ax.transAxes, fontsize=20, fontweight='bold', color= self.cor_principal)
        ...
        sns.despine()
        plt.ylabel("R$ Bilhões", fontsize=12, color=self.cor_secundaria)
        plt.xlabel(None)
        ax.spines[['left', 'bottom']].set_visible(False)
        plt.show()

    def doubleplot(self, X, Y, Y2, titulo:str, subtitulo:str, l1, l2):
        ...
        ax = sns.lineplot(x=X, y=Y, 
                          color=self.cor_principal, linewidth=4, marker='o', label=l1)
        #Plot da linha secundária
        sns.lineplot(x=X, y=Y2, 
                     color=self.cor_secundaria, linewidth=2, linestyle='--', label=l2)

        plt.xticks(X, rotation=45)
        plt.ylabel("R$ Bilhões", fontsize=self.font_eixo, color=self.cor_secundaria)
        plt.xlabel(None)

        ax.text(0.0, 1.12, titulo, 
                transform=ax.transAxes, fontsize=self.font_titulo, fontweight='bold', color= self.cor_principal)
        ax.text(0.00, 1.075, subtitulo, 
                transform=ax.transAxes, fontsize=self.font_subtitulo, color=self.cor_secundaria)

        plt.legend(frameon=False, fontsize=self.font_eixo, loc='upper left', bbox_to_anchor=(0, 1.05), ncol=2)

        plt.figtext(0.585, 0.07, 
                    self.rodape_autoria, 
                    fontsize=self.font_rodape, color=self.cor_secundaria, ha='left')

        sns.despine(left=True, bottom=True)
        plt.subplots_adjust(top=0.82, bottom=0.15, left=0.1, right=0.95)
        plt.show()    

    def plot_acumulado(self, df_anual):
        
        """
        Gera um gráfico de linha da evoluçãao acumulada dos prêmios.
        Recebe um Series ou DataFrame indexado por ano com os valores anuais.

        """
        # Cálculo do acumulado
        df_acumulado = df_anual.cumsum()
        total_geral_bi = df_acumulado.max()
        plt.figure(figsize=self.figsize_padrao)

        #Lineplot do acumulado
        ax = sns.lineplot(x=df_acumulado.index, y=df_acumulado.values, color=self.cor_principal, lw=4)
        ax.text(0.0, 1.12, 'Evolução acumulada dos prêmios', 
                 transform=ax.transAxes, fontsize=22, fontweight='bold', color=self.cor_principal)
        ax.text(0.00, 1.06, f'O histórico acumulado mostra uma aceleração acentuada: o montante total ultrapassou a marca de R$ {total_geral_bi:.1f} bilhões no início de 2026.', 
                 transform=ax.transAxes, fontsize=15, color=self.cor_secundaria)
        ...
        plt.figtext(0.60, 0.07, 
                    self.rodape_autoria, 
                    fontsize=self.font_rodape, color=self.cor_secundaria, ha='left')
        plt.ylabel("Acumulado (R$ Milhões)", fontsize=12, color=self.cor_secundaria)
        plt.xlabel(None)
        plt.show()
    ...
    def plot_convergencia(self, df_hist):
        """
        ...
        color=self.cor_principal, linewidth=4, label='MAE real (desvio observado)')

        sns.lineplot(x=sorteios, y=tendencia_teorica, 
        color=self.cor_secundaria, linewidth=2, linestyle='--', label='Comportamento esperado (1/√N)')

        plt.ylabel("Erro Médio Absoluto", fontsize=self.font_eixo, color=self.cor_secundaria)
        plt.xlabel("Quantidade acumulada de dezenas sorteadas", fontsize=12, color=self.cor_secundaria)
        ...
        ax.text(0.0, 1.12, titulo, transform=ax.transAxes, fontsize=self.font_titulo, fontweight='bold', color=self.cor_principal)
        ax.text(0.00, 1.075, subtitulo,  transform=ax.transAxes, fontsize=self.font_subtitulo, color=self.cor_secundaria)

        plt.legend(frameon=False, fontsize=self.font_eixo, loc='upper left', bbox_to_anchor=(0, 1.05), ncol=2)
        plt.figtext(0.585, 0.07, 
                    'Dados extraídos da Caixa Econômica Federal - (1996-2026) | Elaborado por Lucas Reges Lima', 
                    fontsize=10, color=self.cor_secundaria, ha='left')
        sns.despine(left=True, bottom=True)
        plt.subplots_adjust(top=0.82, bottom=0.15, left=0.1, right=0.95)
        plt.show()

    def plot_hist_dezenas_redline(self, dataset, freq_esp):

        plt.figure(figsize=(16,10))
        ax = sns.histplot(data=dataset, kde=False, bins=60, palette=[self.cor_principal], alpha=0.6, discrete=True, legend=False) 
        ...
        ax.text(0.00, 1.075, 'distribuição das dezenas em relação à frequência esperada', 
                transform=ax.transAxes, fontsize=16, color=self.cor_secundaria)
        ...
        plt.ylabel("Contagem de Sorteios", color=self.cor_secundaria)
    ...
    def plot_hist_dezenas(self, dataset):
        plt.figure(figsize=(16,10))
        ax = sns.histplot(data=dataset, kde=False, bins=60, palette=[self.cor_principal], alpha=0.6, discrete=True, legend=False) 
        plt.xticks(range(1,61,2))
