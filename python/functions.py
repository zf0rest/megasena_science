...
def plot_convergencia_mega(df_hist):
    """
    Calcula e plota a convergência do Erro Médio Absoluto (MAE) da Mega-Sena
    para demonstrar a Lei dos Grandes Números.

    Passe o df_hist contendo o histórico de dezenas sorteadas, apenas. 
    Assume-se que a primeira coluna contém os números sorteados.
    """
    
    n = 60
    prob_teorica = 1 / n #(0,0166)
    erros_medios = [] #isso aqui vai ser o coração do plot, é basicamente o MAE ao longo do tempo 
    sorteios = range(60, len(df_hist) + 1, 10) # A partir do décimo sorteio (porque os primeiros sempre são bagunçados) até o último e de 10 em 10 números.
    ...
    for i in sorteios: #numeros sorteados
        ...
        # Diferença absoluta média em relação à probabilidade teórica
        erro = np.mean(np.abs(freq_relativa - prob_teorica))
        erros_medios.append(erro)

    plt.figure(figsize=(15, 7))
    plt.plot(sorteios, erros_medios, color='#00441b', linewidth=2, label='MAE real')

    # Curva de tendência teórica (1/sqrt(N)) - o erro esperado no acaso
    tendencia_teorica = [erros_medios[0] * (sorteios[0]**0.5) / (n**0.5) for n in sorteios]
    plt.plot(sorteios, tendencia_teorica, color='orange', linestyle='--', alpha=0.6, label='Comportamento esperado (1/√N)')

    plt.title('A Lei dos Grandes números na Mega-Sena: convergência para a uniformidade')

    plt.xlabel('Quantidada acumulada de dezenas sorteadas')
    plt.ylabel('Erro médio (Distância da probabilidade ideal)')
    plt.grid(alpha=0.3)
    plt.legend()
    plt.show()
...
def plot_convergencia(df_hist):
    ...
    seguindo o padrão visual corporativo estabelecido.
    """
    # --- Lógica de Cálculo ---
    n = 60
    prob_teorica = 1 / n #(0,0166)
    ...
    # --- Configuração Visual (Padrão Doubleplot) ---
    sns.set_theme(style="whitegrid")
    plt.figure(figsize=(20, 10))
    
    # Plot da linha principal (MAE Real)
    ax = sns.lineplot(x=sorteios, y=erros_medios, 
                      color='#00441b', linewidth=4, label='MAE Real (Desvio Observado)')
    
    # Plot da linha secundária (Tendência Teórica)
    ...
    # Títulos e Subtítulos no padrão visual
